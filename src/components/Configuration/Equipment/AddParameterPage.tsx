...
const AddParameterPage = () => {
  ...
  const toggleParameterSelection = (index: number) => {
    setSelectedParameterIndices((prev) => {
  ...
  return (
    <Box>
      ...
      <Box sx={{ background: "#FFFFFF", minHeight: "100vh" }}>
          ...
          <Box sx={{ mt: 3, display: "flex", flexWrap: "wrap", gap: 2 }}>
            {parameters.map((p: any, index: number) => (
              <ParameterCard
                ...
                parameter={p}
                index={index}
                isSelected={selectedParameterIndices.includes(index)}
                onToggleSelect={() => toggleParameterSelection(index)}
        ...
        <Box sx={{ height: "1px", background: "#E5E7EB", mt: 2 }} />

        {/* ── Details (Make / Model) — visible only after selecting
             both a unit chip AND a parameter checkbox ── */}
        {showDetails && (
          <MakeModelSection
            selected={selected}
            equipmentName={equipmentName}
...
/* ── Make / Model shared for all selected units ── */
    ...
    <table style={{ width: "100%", borderCollapse: "collapse" }}>
      <thead>
        <tr style={{ background: "#F9FAFB", height: "42px" }}>
          ...
          <th style={headerStyle}>No. of Parameters</th>
        </tr>
      </thead>
      <tbody>
