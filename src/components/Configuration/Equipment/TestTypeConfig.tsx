...
const TestTypeConfig: React.FC<TestTypeConfigProps> = ({
  readOnly = false,
}) => {
  ...
  const onSubmit = async (data: any) => {
    try {
      if (editing) {
        setTestTypes(
          testTypes.map((t) => (t.id === editing.id ? { ...t, ...data } : t)),
        );
      } else {
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
                <TextField
                  ...
                  {...register("test_type_name", {
                    required: "Test type name is required",
                  })}
