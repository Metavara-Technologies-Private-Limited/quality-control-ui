    ...
    <div style={{ display: "flex", minHeight: "100vh", backgroundColor: "#f9fafb", padding: "20px", gap: "20px" }}>
      ...
      <div style={{ 
        ...
        <div style={{ padding: "20px", borderBottom: "1px solid #e5e7eb", display: "flex", justifyContent: "space-between", alignItems: 'center' }}>
          <h2 style={{ fontSize: "18px", fontWeight: "600", margin: 0, color: "#0f172a" }}>Equipments</h2>
            ...
            andrologyList.map((item) => (
              ...
              >
                    ...
                    <div style={{ display: "flex" }}>
                      {assignees.map((img, i) => (
                        <img 
                          ...
                          src={img} 
                          alt="assignee" 
                          style={{ width: "20px", height: "20px", borderRadius: "50%", border: "2px solid white", marginLeft: i > 0 ? "-6px" : 0 }} 
                        />
                      ))}
                    </div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: 'center' }}>
                  <span style={{ fontSize: "12px", color: item.progress === "100%" ? "#22c55e" : "#f97316", fontWeight: "600" }}>
                    {item.progress}
                  </span>
                  <span style={{ fontSize: "12px", color: "#64748b" }}>Parameters : {item.params}</span>
                </div>
              </div>
            ))
          ) : (
      ...
      <div style={{ width: "994px", height: "879px" }}>
        {activeTab === "To-Do" ? (
          <>
            {equipmentType === "sperm" && (
              <SpermAnalyzersForm selectedRadio={selectedRadio} setSelectedRadio={setSelectedRadio} />
            )}
            {equipmentType === "centrifuge" && (
              <CentrifugesForm selectedRadio={selectedRadio} setSelectedRadio={setSelectedRadio} />
            )}
            {equipmentType === "autoclave" && (
              <AutoclavesForm selectedRadio={selectedRadio} setSelectedRadio={setSelectedRadio} />
            )}
            {equipmentType === "gas" && (
              <GasAnalyzersForm selectedRadio={selectedRadio} setSelectedRadio={setSelectedRadio} />
            )}
            {equipmentType === "fridge" && (
              <RefrigeratorFreezerForm selectedRadio={selectedRadio} setSelectedRadio={setSelectedRadio} />
            )}
            ...
            {!equipmentType && (
               <div style={{ 
                 ...
                 height: "419px", backgroundColor: "#fff", borderRadius: "12px", 
          ...
          <div style={{ 
            display: "flex", alignItems: "center", justifyContent: "center", 
            height: "419px", backgroundColor: "#fff", borderRadius: "12px", 
            border: "1px solid #e5e7eb", color: "#94a3b8", fontSize: "16px" 
          }}>
            No plans added yet
          </div>
        )}
