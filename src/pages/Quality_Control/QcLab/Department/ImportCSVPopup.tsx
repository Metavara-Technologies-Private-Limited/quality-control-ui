...
export default function ImportCSVPopup({
  open,
  onClose,
  parameters,
  onImport,
  ...
  const parameterUnitMap = new Map(
    parameters.map((p) => [p.parameter_name, p.config?.unit || "-"]),
  );
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedExtensions.includes(fileExtension || "")) {
      setErrors([
        "Unsupported file format. Please upload only CSV or Excel (.xlsx, .xls) files.",
      ]);
      setFileName("");
      event.target.value = "";
      return;
    ...
    if (fileExtension === "csv") {
      ...
      reader.onload = (e) => {
        try {
          ...
          setErrors([
            "Failed to read Excel file. Please ensure it is not password protected.",
          ]);
        }
        setIsProcessing(false);
      };
  ...
  const validateAndProcessCSV = (data: any[]) => {
    ...
    if (!headers.includes("Parameter"))
      validationErrors.push("Missing 'Parameter' column");
    if (!headers.includes("Value"))
      validationErrors.push("Missing 'Value' column");
    if (!headers.includes("Equipment"))
      validationErrors.push("Missing 'Equipment' column");
    if (!headers.includes("Date & Time"))
      validationErrors.push("Missing 'Date & Time' column");
    ...
    data.forEach((row, index) => {
      ...
      if (!rowEquipment) {
        validationErrors.push(
          `Row ${rowNum}: no Equipment added please check?`,
        );
      }
      ...
      if (matchedLabel) {
        ...
        if (rowEquipment && row["Date & Time"] && row["Value"] !== undefined) {
          validRows.push({
            ...row,
            Parameter: matchedLabel,
          });
        }
      } else if (rowParamValue) {
        validationErrors.push(
          `Row ${rowNum}: "${rowParamValue}" is not a recognized parameter.`,
        );
      }

      if (!row["Date & Time"])
        validationErrors.push(`Row ${rowNum}: Missing Date & Time`);
    });

    // 4. MISSING PARAMETER CHECK
    const missing = parameterLabels.filter(
      (label) => !foundParamsInCSV.has(label),
    );
    missing.forEach((m) => {
          ...
          >
            ...
            <Box sx={{ maxHeight: "120px", overflowY: "auto" }}>
              {errors.map((err, i) => (
                <Typography
                  key={i}
                  sx={{ fontSize: "12px", color: "#B91C1C", display: "block" }}
                >
                  • {err}
                </Typography>
              ))}
        ...
        {missingParams.length > 0 && csvData.length > 0 && (
          <Alert severity="warning" sx={{ mt: 2 }} icon={<WarningAmber />}>
            ...
            <Typography variant="caption">
              Some parameters (<strong>{missingParams.join(", ")}</strong>) are
              missing from the file. Matched data will still be imported.
            </Typography>
          </Alert>
        )}
          ...
          <Box sx={{ mt: 3 }}>
            ...
            >
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                >
                  <tr>
                    {["Date & Time", "Equipment", "Parameter", "Value"].map(
                      (h) => (
      ...
      <DialogActions sx={{ p: 3 }}>
        ...
        <Button
          ...
          sx={{
            ...
            "&:hover": {
              bgcolor: missingParams.length > 0 ? "#d16d51" : "#232323",
            },
          }}
        >
          {missingParams.length > 0
            ? "Yes, Import Matched Data"
            : "Add to Logs"}
        </Button>
      </DialogActions>
    </Dialog>
