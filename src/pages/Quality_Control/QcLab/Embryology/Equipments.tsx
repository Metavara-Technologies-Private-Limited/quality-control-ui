...
const Equipment = () => {
  ...
  const determineEquipmentType = (name) => {
  ...
  if (view === "list") {
    // List view remains the same with To-Do/Plan tab logic
    return (
                ...
                <div key={item.name} onClick={() => { setSelectedEquipment(item.name); setSelectedRadio(item.name); setEquipmentType(determineEquipmentType(item.name)); setView("detail"); }} style={{ backgroundColor: "#fff", border: "1px solid #e5e7eb", borderRadius: "12px", padding: "16px", cursor: "pointer" }}>
                   ...
                   <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                      {assignees.map((img, i) => <img key={i} src={img} style={{ width: "24px", height: "24px", borderRadius: "50%", border: "2px solid white", marginLeft: i > 0 ? "-8px" : 0 }} />)}
                      <div style={{ width: "24px", height: "24px", borderRadius: "50%", backgroundColor: "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center" }}><Plus size={14} color="#64748b" /></div>
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span style={{ fontSize: "13px", color: "#22c55e", fontWeight: "600" }}>{item.progress}</span>
                    <div style={{ display: "flex", gap: "8px" }}>
                      {item.tags.map((tag, i) => <span key={i} style={{ backgroundColor: tag.color, color: "#fff", padding: "4px 12px", borderRadius: "12px", fontSize: "11px" }}>{tag.text}</span>)}
                    </div>
                  </div>
                </div>
              ))}
    ...
    <div style={{ display: "flex", minHeight: "100vh", backgroundColor: "#f9fafb", padding: "20px", gap: "20px" }}>
      ...
      <div style={{ width: "512px", height: "840px", backgroundColor: "#fff", border: "1px solid #e5e7eb", borderRadius: "14px", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "20px", borderBottom: "1px solid #e5e7eb", display: "flex", justifyContent: "space-between" }}>
          <button onClick={() => setView("list")} style={{ background: "none", border: "none", fontSize: "14px", fontWeight: "500", cursor: "pointer", color: "#0f172a" }}>← Equipments</button>
        ...
        <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "20px", overflowY: "auto", flex: 1 }}>
          ...
          {/* UPDATED CONDITIONAL SIDEBAR CONTENT */}
          {activeTab === "To-Do" ? (
              ...
              <div key={item.name} onClick={() => { setSelectedEquipment(item.name); setEquipmentType(determineEquipmentType(item.name)); setSelectedRadio(item.name); }} style={{ padding: "16px", borderRadius: "12px", cursor: "pointer", backgroundColor: selectedEquipment === item.name ? "#fef3f2" : "#fff", border: selectedEquipment === item.name ? "2px solid #f97316" : "1px solid #f1f5f9" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                    <div style={{ display: "flex" }}>
                      {assignees.slice(0, 3).map((img, i) => <img key={i} src={img} style={{ width: "20px", height: "20px", borderRadius: "50%", border: "1px solid white", marginLeft: i > 0 ? "-6px" : 0 }} />)}
                    </div>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  <div style={{ display: "flex", gap: "4px" }}>
                    {item.tags.map((tag, i) => <span key={i} style={{ backgroundColor: tag.color, color: "#fff", padding: "2px 8px", borderRadius: "10px", fontSize: "10px" }}>{tag.text}</span>)}
                  </div>
                </div>
              </div>
            ))
