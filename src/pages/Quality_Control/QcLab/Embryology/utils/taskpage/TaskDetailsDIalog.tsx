  ...
  import { SubTask, Task, TaskUpdatePayload } from '@/types';
...
import { taskApi } from '@/services/api';
  ...
  const TaskDetailsDialog: React.FC<TaskDetailsDialogProps> = ({
    ...
    useEffect(() => {
      if (open && task) {
        setTaskStatus(task.status_label || 'To Do');
  ...
  >
    ...
    <>
      <Avatar sx={{ width: 24, height: 24 }}>
        {assignedUser.emp_name?.[0]}
      </Avatar>
      <Typography fontWeight={500}>
        {assignedUser.emp_name}
      </Typography>
    </>
  ) : (
        ...
        <Box sx={{ border: '1px solid #E0E0E0', borderRadius: '16px', p: 3 }}>
            ...
            <Box flex={1}>
              ...
              <FormControl fullWidth size="small" >
                <Select
                  ...
                  onChange={(e) =>
                    setNewSubTask({ ...newSubTask, status: Number(e.target.value) }) // Convert to number
                  }
                  sx={{ borderRadius: '8px' }}
                >
          ...
          <Stack direction="row" spacing={3} mb={3}>
            <Box flex={1}>
              ...
              <DatePicker
                ...
                onChange={(val) => setNewSubTask({ ...newSubTask, due: String(val) })}
                format="DD/MM/YYYY"
                slotProps={{
                  textField: {
                    fullWidth: true,
          ...
          <Box display="flex" justifyContent="flex-end">
            <Button
              ...
              onClick={handleSaveDetails} // same handler can handle both add & edit
            >
              Save
            </Button>
          </Box>
