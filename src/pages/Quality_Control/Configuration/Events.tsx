    ...
    <Card sx={{ borderRadius: "12px", border: `1px solid ${COLORS.border}`, boxShadow: "none" }}>
      <Box p={3} borderBottom={`1px solid ${COLORS.bgChip}`}>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Typography fontSize={20} fontWeight={700} color={COLORS.textPrimary}>{event.name}</Typography>
          <Stack direction="row" spacing={3} alignItems="center">
            ...
            <Stack direction="row" spacing={1} alignItems="center">
              ...
              <Typography fontSize={12} color={COLORS.textPrimary}>{event.createdBy}</Typography>
            </Stack>
          </Stack>
        </Stack>
      </Box>
      ...
      <Box p={3.5}>
        <Typography fontSize={14} fontWeight={700} mb={3} color={COLORS.textPrimary}>Event Name</Typography>
        <Grid container spacing={3} mb={5}>
          {/* Label positioning updated to overlap border per Figma */}
          <Grid item xs={12} md={5}>
            <Box sx={{ position: "relative" }}>
              ...
              <Box sx={{ p: 2, borderRadius: "8px", border: `1px solid ${COLORS.border}`, minHeight: "48px" }}>
        ...
        <Box sx={{ display: 'inline-flex', alignItems: 'center', bgcolor: COLORS.bgChip, px: 2, py: 0.8, borderRadius: '24px', border: `1px solid ${COLORS.border}` }}>
          ...
          <Typography fontSize={14} color={COLORS.textPrimary}>{event.createdBy}</Typography>
        </Box>
      </Box>
    </Card>
  </Box>
...
const EventsTable = ({
  ...
  return <Card sx={{ borderRadius: "12px", border: `1px solid ${COLORS.border}`, boxShadow: "none" }}>
    <Table>
      <TableHead sx={{ bgcolor: COLORS.bgLight }}>
        <TableRow>
          ...
          <TableCell align="center" sx={{ fontWeight: 600 }}>Total No Equipment</TableCell>
          <TableCell align="center" sx={{ fontWeight: 600 }}>Total No Parameters</TableCell>
        </TableRow>
      </TableHead>
      <TableBody>
        {data.map((row: any) => (
      ...
      <Stack direction="row" spacing={1} alignItems="center">
        ...
        {Array.from({ length: totalPages }, (_, i) => (
          <Button
            ...
            sx={{
              minWidth: 32, height: 32, borderRadius: "4px",
...
const mapEventToRow = (e: any) => ({
  ...
  days: e.schedule?.days && e.schedule.days.length > 0 ? e.schedule.days.join(", ") : "-",
...
const Events = () => {
  ...
  useEffect(() => {
    if (clinic?.id) { dispatch(fetchEventsByClinic(clinic.id)); }
  }, [clinic?.id, dispatch]);  
  ...
  return (
    <Box sx={{ minHeight: "100vh" }}>
      {viewingEvent ? (
        ...
        <>
          <EventsHeader onCreate={() => navigate("/configuration/events/create")} onSearch={(val: string) => { setSearch(val); setPage(0); }} />
          {eventLoading ? (
            <Card sx={{ borderRadius: "12px", border: `1px solid ${COLORS.border}`, boxShadow: "none", p: 4, textAlign: "center" }}>
              <Typography fontSize={14} color={COLORS.textMuted}>Loading events...</Typography>
            </Card>
          ) : (
            <EventsTable data={paginatedEvents} page={page} rowsPerPage={rowsPerPage} total={filteredEvents.length} onPageChange={setPage} onRowClick={setViewingEvent} />
          )}
        </>
      )}
    </Box>
